...
(function() {
  ...
  if (document.addEventListener) {
	...
	setTimeout( ' (function(){ document.fire("dom:loaded");document.loaded = true; })() ', 1000 );
...
var keyCode = {
  ...
  getKeyStatus : function( e ){
    ...
    if( Prototype.Browser.IE ) e = event; // IEの場合

    // 入力文字取得
    if( Prototype.Browser.IE ){ // onkeydownからやってくる
      inputCode = e.keyCode;//( Prototype.Browser.IE ? e.keyCode : e.charCode ); // IEだとkeyCode
    ...
    else if( Prototype.Browser.Gecko ){// Firefox onkeypressからやってくる
    ...
    else if( Prototype.Browser.WebKit ){ // Webkit
      inputCode = e.keyCode;
      if( this.isAlphabetUpper( inputCode ) ){
        inputChar = String.fromCharCode(inputCode);
...
JS_IM.prototype = {
  ...
  initialize : function(formObj,methodObj){
    ...
    var keyProcess = function(e){
      if( !Prototype.Browser.IE && !event ) var event = e;
    ...
    */
    ...
    else if( Prototype.Browser.Gecko ){ // Gecko
...
*/
      ...
      update : function( bufferStr ){
        ...
        this.elem.style.width = ( bufferStr.stripTags().length * 18 ) + "px";
        this.flush();
      },
      flush : function(){
        this.elem.style.visibility = "visible";
...
http://la.ma.la/js/roma.js
 ...
 - 1.0 ( 2005-04-27 ) : 公開
 - 2.0 ( 2005-09-20 ) : コードをかなり短く。
  インスパイア http://www.karlson.ru/jstoys/index.php?module=input_ja
  改造と高速化 by ma.la
*/
var roma2 = {};
